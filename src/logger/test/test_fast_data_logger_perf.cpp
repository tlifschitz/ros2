...
protected:
  void SetUp() override
  {
    ...
    if (is_containerized_) {
      std::cout << "Detected containerized environment - adjusting performance expectations" <<
        std::endl;
    }
  }
...
TEST_F(LoggerPerformanceTest, HighThroughputTest)
{
  ...
  double throughput_mb = (num_messages * message_size) / 1024.0 / 1024.0 /
...
TEST_F(LoggerPerformanceTest, ConcurrentLoggingTest)
{
  ...
  for (int t = 0; t < num_threads; ++t) {
    threads.emplace_back(
      [this, messages_per_thread, &test_message, t]() {
        for (int i = 0; i < messages_per_thread; ++i) {
          std::string msg = test_message + " from thread " + std::to_string(
            t) + " msg " + std::to_string(i);
  ...
  std::cout << "Expected min concurrent rate: " << min_concurrent_rate << " messages/sec" <<
    std::endl;
  ...
  if (std::filesystem::exists("./logs")) {
    for (const auto & entry : std::filesystem::directory_iterator("./logs")) {
      if (entry.path().extension() == ".bin") {
        ...
        std::cout << "Log file: " << entry.path() << " (" << total_file_size << " bytes)" <<
          std::endl;
        break;
      }
    }
